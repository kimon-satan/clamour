...
blobFragmentShader = `
  ...
  varying vec2 o_uv;
	...
	vec3 tex2D(vec2 p){
		...
		float fgl = pow(clamp(cellTex(p) * 4.0, 0.0, 1.0), fg_pow);
		float hll = dot(sin(p * hl_freq - sin(p.yx * hl_freq + fgl * hl_freq * 1.5)), vec2(.5));
		hll = clamp(pow(hll, hl_pow), 0.,1.);

		vec3 c_mix = mix(bg_color, hl_color, hll * hl_mul);
	...
	void main()	{
		...
		float v = (cos(ustheta * TWO_PI * edge_freq - c_time) + 1.0)/2.0;

		o_edge += (pow(v, 1.0) * 2.0 -1.0 ) * edge_amp * pow(sin(abs(theta)), 2.0);
		o_edge = clamp(o_edge, 0., 1.);
		//o_edge += cos(theta * 2.0 + PI/2.0)  * edge_amp;
		float c_edge = c_size * 0.7 + c_amp * 0.3 * c_size * c_noise;
		//c_edge -= sin(ustheta * PI * edge_freq)  * edge_amp;
